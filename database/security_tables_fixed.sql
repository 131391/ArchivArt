-- Security Tables Migration (Fixed)
...
SET @sql = (SELECT IF(
    ...
    'SELECT "Column last_activity_at already exists"'
));
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @sql = (SELECT IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS 
     WHERE TABLE_SCHEMA = 'archivart' 
     AND TABLE_NAME = 'users' 
     AND COLUMN_NAME = 'verification_token_expires') = 0,
    'ALTER TABLE users ADD COLUMN verification_token_expires TIMESTAMP NULL',
    'SELECT "Column verification_token_expires already exists"'
));
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @sql = (SELECT IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS 
     WHERE TABLE_SCHEMA = 'archivart' 
     AND TABLE_NAME = 'users' 
     AND COLUMN_NAME = 'reset_password_token') = 0,
    'ALTER TABLE users ADD COLUMN reset_password_token VARCHAR(255) NULL',
    'SELECT "Column reset_password_token already exists"'
));
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @sql = (SELECT IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS 
     WHERE TABLE_SCHEMA = 'archivart' 
     AND TABLE_NAME = 'users' 
     AND COLUMN_NAME = 'reset_password_expires') = 0,
    'ALTER TABLE users ADD COLUMN reset_password_expires TIMESTAMP NULL',
    'SELECT "Column reset_password_expires already exists"'
));
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
